...
class PIDArduinoLogic(Automatic):
    ...
    KEY_KI = "Ki"
    KEY_KD = "Kd"
    KEY_MAXOUT = "power limit %"
    SAMPLETIME = 5

    configparameter = [
            {"name": KEY_KP, "value": 107},
            {"name": KEY_KI, "value": 0.9},
            {"name": KEY_KD, "value": 202},
            {"name": KEY_MAXOUT, "value": 100}]

    def run(self):
        wait_time = self.SAMPLETIME
        kp = float(self.config[self.KEY_KP])
        ki = float(self.config[self.KEY_KI])
        kd = float(self.config[self.KEY_KD])
        ...
        while self.isRunning():
            ...
            # Turn heater on/off only when necessary to avoid SSR "flickering"
            if heating_time > 0:
                self.switchHeaterON()

...
def initGPIO():
    try:
        ...
        for vid in app.brewapp_kettle:
            if(app.brewapp_kettle[vid]["heater"]["gpio"] != None):
                print "SETUP GPIO HEATER", app.brewapp_kettle[vid]["heater"]["gpio"]
                GPIO.setup(int(app.brewapp_kettle[vid]["heater"]["gpio"]), GPIO.OUT)
                GPIO.output(app.brewapp_kettle[vid]["heater"]["gpio"], 1)
            if(app.brewapp_kettle[vid]["agitator"]["gpio"] != None):
                print "SETUP GPIO AGITATOR", app.brewapp_kettle[vid]["agitator"]["gpio"]
...
def toogle(vid, name, gpio):
    if(app.brewapp_kettle[vid].get(name).get("gpio") == gpio):
        if(app.brewapp_kettle[vid].get(name).get("state") == False):
            switchON(gpio)
            app.brewapp_kettle[vid].get(name)["state"] = True
        else:
            switchOFF(gpio)
            app.brewapp_kettle[vid].get(name)["state"] = False
